  ...
  return (
    <tr className="align-top transition hover:bg-[#fffafb]">
      <td className="px-4 py-4">
        <div className="font-semibold text-[#5f4d56]">{lead.name}</div>
      ...
      <td className="px-4 py-4 text-xs text-gray-600">
        <div className="space-y-1">
          ...
          {lead.source ? (
            <p className="max-w-[220px] truncate">
              <span className="font-semibold">Src:</span> {lead.source}
            </p>
          ) : null}
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        ...
        />
        {lead.contacted_at || lead.booked_at ? (
          <p className="mt-2 text-xs text-gray-500">
            {lead.contacted_at ? `Contactado: ${formatDate(lead.contacted_at)}` : ""}
            {lead.booked_at ? ` · Reservado: ${formatDate(lead.booked_at)}` : ""}
          </p>
        ) : null}
      </td>
